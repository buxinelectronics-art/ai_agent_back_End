...
def upload_resume():
    ...
    if not _allowed(f.filename):
        return jsonify(error="PDF/DOCX only"), 400
...
def generate():
    ...
    generate_resume_task(app, uid, job_id)
    generate_cover_letter_task(app, uid, job_id)
    return jsonify(message="Generating in background — check /resume/list shortly"), 202
...
def list_docs():
    uid = int(get_jwt_identity())
    r = Resume.query.filter_by(user_id=uid).order_by(
        Resume.generated_at.desc()).limit(50).all()
    c = CoverLetter.query.filter_by(user_id=uid).order_by(
        CoverLetter.generated_at.desc()).limit(50).all()
    return jsonify(
        resumes=[{
            "id": x.id, "job_title": x.job_title, "company": x.company,
            "ats_score": x.ats_score, "generated_at": x.generated_at.isoformat()
        } for x in r],
        cover_letters=[{
            "id": x.id, "company": x.company,
