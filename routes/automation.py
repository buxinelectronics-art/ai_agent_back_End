...
def start():
    ...
    if not s.is_running:
        ...
        db.session.add(ActivityLog(
            user_id=uid, event_type="system", message="Automation started"))
        db.session.commit()
        # Start first cycle in background thread
...
def stop():
    ...
    s.is_running = False
    db.session.add(ActivityLog(
        user_id=uid, event_type="system", message="Automation stopped"))
    db.session.commit()
